		...
		<table class="orderbox-table">
			...
			<tr>
				...
				<td class="orderbox-td-tc">
					...
					<p class="sixfont">{{if $value.adjustFreight == null}}（物流未设定）{{else}}（物流：{{$formatMoney $value.adjustFreight}}）{{/if}}</p>
				</td>
				<td class="orderbox-td-tc">
				...
				<td class="orderbox-td-tc">
					{{if $value.orderStatus == 1}}
						<div><a href="javascript:;" class="ui_button ui_button_sblue js_update_cash" data-orderid="{{$value.refrenceId}}" data-brandsid="{{$value.brandsId}}" data-type="price">修改金额</a></div>
					...
					{{/if}}
					{{if $value.orderStatus != 9 && $value.orderStatus != 4 && $value.orderStatus != 3 && $value.orderStatus != 10}}
    ...
    <div class="ui-head">
      ...
      <form:form action="#" id="updatePrice">
        ...
        <div class="cash_info">
            ...
            <div class="item">
                <label for="">实付款：</label>
                <strong class="price">{{price}}</strong><span class="price_explain">（货款）</span>
               <%-- <span class="symbol">{{if oldAdjustPrice>0}}+{{else}}-{{/if}}</span>
                <strong class="price prefer_price total_discount">{{ adjustPrice || 0 }}</strong>
				<span class="price_explain">（<span class="symbol_text">{{if oldAdjustPrice>0}}加价{{else}}优惠{{/if}}</span>）</span>--%>
                + <strong class="price total_wuliu">{{ adjustFreight || 0 }}</strong><span class="price_explain">（物流费）</span>=
...
<script>
   ...
   $(function () {
       ...
       seajs.use(["pagination","moment","template"], function (Pagination,moment,template) {
           ...
           page = new Pagination({
               url: "${ctx}/brand/order/factory/data",
               elem: "#pagination",
               form:$("#orderForm"),
               method:"post",
   ...
   TradePurorder.updatePop = function(orderID,dialog){
       $('#updatePrice input[name="refrenceId"]').val(orderID);
       $.ajax({
           type:"post",
