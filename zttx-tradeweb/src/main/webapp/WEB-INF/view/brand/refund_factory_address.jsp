                    ...
                    <div class="detail_box">
                        ...
                        <p class="explain mt10"><strong class="c-r time"  data-endtime="${endTime}"  style=" color:red"></strong>之后系统会将${dealerRefund.refundAmount}元货款打到终端商的账户中</p>
                            ...
                            <p class="explain">您还有 <strong class="c-r time" data-endtime="${endTime}"
                                                           style=" color:red"></strong> 来处理终端商的退货退款申请</p>
                            ...
                            <div class="detail">
                                ...
                                <div class="item">
                                    ...
                                    <div class="inline-block">
                                        ...
                                        <c:forEach items="${dealerRefAttachts}" var="dealerRefAttacht">
                                            <a href="${res}${dealerRefAttacht.domainName}${dealerRefAttacht.attachtName}"
                                               data-lightbox="arraylist_${status.index}">
                                                <img width="100" height="100" src="${res}${dealerRefAttacht.domainName}${dealerRefAttacht.attachtName}"/>
                                            </a>
                                        </c:forEach>
                                ...
                                <c:if test="${not empty defaultBrandAddress}">
                                    <div class="item return_address">
                                        ...
                                        <input id="brandRecAddr" name="brandRecAddr'" style="width: 350px;" class="ui-input"
                                               value="${defaultBrandAddress.userName} ${defaultBrandAddress.fullAreaName}${defaultBrandAddress.street}${defaultBrandAddress.userMobile}${empty defaultBrandAddress.userTel?'':'/'}${defaultBrandAddress.userTel}${empty defaultBrandAddress.zipCode?'':'/' }${defaultBrandAddress.zipCode}"
                                               disabled/>
                                    ...
                                    <div class="operation">
                                        ...
                                        <a class="ui_button ui_button_mgray ml5" onclick="order_refuse();">拒绝</a>
                                    </div>
                                </c:if>
                                <c:if test="${empty defaultBrandAddress}">
                                    <a href="${ctx}/brand/brandAddress/execute" target="_blank"
                                       style="background: #21AD39; color: #fff; padding: 5px;">请先添加收货地址</a>
                                </c:if>
                            </div>
                        </div>
                    </div>
...
<form:form id="order_refuse" action="${ctx}/brand/refund/factory/refuse/${dealerRefund.refundId}" method="post"></form:form>
...
<script>
    ...
    var refundDetail_address = {
        ...
        handleAddress: function (dialog) {
            ...
            $("#brandRecAddr").val($.trim($("input:checked").parent().parent().children("td").eq(3).text()) + " "
            + $.trim($("input:checked").parent().parent().children("td").eq(4).text()) + " "
            + $.trim($("input:checked").parent().parent().children("td").eq(5).text()) + " "
            + $.trim($("input:checked").parent().parent().children("td").eq(6).text()));
