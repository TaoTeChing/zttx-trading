					...
					<div class="top clearfix">
						<div class="left">
							<div class="refund_info">
								<h3>退款信息</h3>
								<p>
									<label>退款单号：</label><span class="num">${dealerRefund.refundId}</span>
								</p>
								<p>
									<label>退款类型：</label>${dealerRefund.needRefund==true ?"退款退货":"仅退款"}
								</p>
								<p>
									<label>退款金额：</label><strong class="price">${dealerRefund.refundAmount}</strong>元
								</p>
							</div>
							<div class="order_info">
								<h3>订单信息</h3>
								<p>
									<label>订单号：</label><a href="${ctx}/brand/order/view/${dealerOrder.refrenceId}" target="_blank" class="link">
								...
								<p>
									<label> 订货数量：</label>${dealerOrder.productCount}
								</p>
								<p>
									<label>订货金额：</label><strong class="price">${dealerOrder.payment}</strong>元
								</p>
								<p>
									<label>物流费用：</label>${dealerOrder.adjustFreight}元
								</p>
								<p>
									<label>终端商：</label>${dealerOrder.dealerName}
								</p>
							</div>
						</div>
						<div class="detail_box">
						  <%-- 已要求客服介入,等待客服介入 --%>
							...
							<div class="handle_refund">
								<h3>终端商请求退款</h3>
								<p class="explain">
									您还有 <strong class="c-r time" data-endtime="${endTime}"   style=" color:red"></strong> 来处理终端商的退款申请
								</p>
								 <form:form action="" id="subPass_Form">
								<div class="detail">
									<p class="item">
										<label>退款原因：</label>${dealerRefund.refundReason}
									</p>
									<p class="item">
										<label>退款金额：</label><strong class="price">${dealerRefund.refundAmount}</strong>
									</p>
									<p class="item">
										<label>退款说明：</label>${dealerRefund.refundMark}
									</p>
									<div class="item">
										<label>凭证：</label>
                  	...
                  	src="${res}${dealerRefAttacht.domainName}${dealerRefAttacht.attachtName}" />
									...
									<div class="item">
										<input type="hidden" id="refrenceId"
											...
											<c:if test="${hasBrandPayword==true}"><label style="vertical-align: middle"  >平台支付密码：</label>
                                        ...
                                        <!-- <button style="width: auto;margin-left: 65px;" id="setPass_btn" class="simple_button">请先设置支付密码</button> -->
                                        <a href="<%=ZttxConst.PAYAPI_WEBURL_RESETPWD%>" style="width: auto;margin-left: 65px;" class="simple_button" target="_blank">请先设置支付密码</a>
									...
									<div class="operation">
										...
										<a class="ui_button ui_button_mgray ml5"
											href="${ctx}/brand/refund/view/${dealerRefund.orderNumber}">取消</a>
									</div>
								</div>
								 </form:form>
							</div>
    ...
    <div style="display: none;">
        <div id="setPassword_box" class="setPassword_box">
            ...
            <form:form data-widget="validator" class="ui-form setPass_Form mt15" id="setPass_Form">
                ...
                <div class="ui-form-item">
                    <label class="ui-label">重复支付密码</label>
