...
<body>
    ...
    <div class="main_con">
        ...
        <div class="inner">
            <!-- 内容都放这个地方  -->
            <form:form method="get" class="ui-form proline-step2-form clearfix" id="pagination-form">
                ...
                <div class="ui-form-item inline-block">
                    <input type="button" class="simple_button fl" value="搜索" onclick="page.goTo(1);"/>
                </div>
            </form:form>
            <div class="js_addline2_menu clearfix">
                <ul>
					<li>
						<a href="${ctx}/brand/line/${line.refrenceId}/unadd">未选择（${totals - listCount}）</a>
					</li>
					<li class="selected">
						<a href="javascript:void(0);">已选择（${listCount}）</a>
					</li>
				</ul>
            </div>
            <div class="js_addline2_con">
                <div class="js_addline2_conbox">
                    <div class="stepT-checkallbar">
                        ...
                        <a class="stepT-cb-a" id="batchDel">批量移除</a>
                        <a class="">批量设置折扣率：
                            <input type="text" class="ui-input ratetext js-price"/>折</a>
                        <a class="stepT-cb-sure stepT-nocb-sure">确定</a>
                        <a id="btn-submit" href="javascript:;" class="stepT-cb-sure ml5" style="display:none;">批量加入产品线</a>
                    </div>
                    ...
                    <form:form id="remove-form" action="" method="post" cssStyle="display: none;">
                    </form:form>
                </div>
            </div>
            <!-- 批量抹零 -->
...
<ul class="clearfix">
	<li>
		<a href="/brand/line/${line.refrenceId}/unadd">未选择（{{object.notSelCount}}）</a>
	</li>
	<li class="selected">
		<a href="javascript:void(0);">已选择（{{object.selCount}}）</a>
	</li>
</ul>
</script>
...
<tbody>
	...
	<tr>
		<td class="td-000">
            ...
            <div class="fl list-TD-imgbox">
			    <img src="${res}{{$getImageDomainPathFn $value.domainName+$value.productImage 160 160}}" width="70" height="70" alt=""/>
                {{if $value.beginType!=1}}
				<div class="recom-product-off">已下架</div>
				{{/if}}
            </div>
		...
		<td class="td-002">
			<input type="text" class="proline-stepT-si noreteval js-price" placeholder="3.5" value="{{$formatMoney $value.productAgio*10}}"/>&nbsp;折
		</td>
		<td class="td-003">
			<span class="fn-rmb">￥</span> <span class="nprice">{{$formatMoney $value.productPrice*$value.productAgio}}</span>
		</td>
	</tr>
    {{/each}}
</tbody>
...
<script type="text/javascript">
    $(function () {
                ...
                $.ajax({
					...
					success : function(json) {
						if(zttx.SUCCESS==json.code){
							remind("success","产品折扣率已修改成功",function(){
								page.render();
							}); 
						}else
        ...
        $("#batchDel").on("click", function(){
        	...
        	if($inputs.length>0)
        	{
        		confirmDialog("是否批量移除产品？",function(){
	        		$addForm.html("");
	        		$inputs.each(function (index) {
	                    var $this = $(this);
	                    var id = $this.val();
	                ...
	                $.ajax({
						type : "POST",
						url : "${ctx}/brand/line/${line.refrenceId}/delete",
						data:$("#remove-form").serialize(),
						dataType: "json",
						success : function(json) {
							if(zttx.SUCCESS==json.code){
								remind("success","产品已成功移除",function(){
									page.render();
								}); 
							}else
								remind("error",json.message); 
  ...
  $(function () {
    seajs.use(["pagination","template","jscroll"], function (Pagination,template) {
        ...
        var renderPagination = function(){
       	page = new Pagination({
             url: "${ctx}/brand/line/${line.refrenceId}/added.json",
             elem: "#pagination",
