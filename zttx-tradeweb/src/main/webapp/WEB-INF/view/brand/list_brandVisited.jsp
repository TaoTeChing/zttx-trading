...
<head>
<meta charset="UTF-8">
<title>管理中心-终端商管理-我浏览过的终端商</title>
	...
	<table id="tempTable" class="agency-seeking-table">
		...
		{{each rows}}
			<tr class="{{$index%2==0 ? '':'odd'}}">
				...
				<!--<td class="ta-c"><span class="agency-seeking-td-exp">{{if $value.monNum==null}}--{{else}}{{$value.monNum}}{{/if}}</span></td>-->
				...
				<td class="ta-c"><span class="agency-seeking-td-exp">{{if $value.foundTime==null || $value.foundTime<=0}}--{{else}}{{$formatDate $value.foundTime}}{{/if}}</span></td>
				<td>
	...
	<script>
	  ...
	  $(function () {
	    seajs.use(["pagination","moment"], function (Pagination,moment) {
        	...
        	page = new Pagination({
              url: "${ctx}/brand/dealer/brandVisited/data",
              elem: "#pagination",
