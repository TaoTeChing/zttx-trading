...
<head>
<meta charset="UTF-8">
<title>管理中心-终端商管理-平台推荐的终端商</title>
	...
	<table id="tempTable" class="agency-seeking-table">
		...
		{{each rows}}
			<tr class="{{$index%2==0 ? '':'odd'}}">
				<td>
					<span style="width: 105px;" class="fl fn-text-overflow" title="{{$value.dealerName}}">{{$value.dealerName}}</span>
                    <div class="fr">
						{{if $value.isExist}}
				...
				<td class="ta-c"><span class="agency-seeking-td-exp">{{$value.brandsName}}</span></td>
	...
	<script>
	  ...
	  $(function () {
	    seajs.use(["pagination","moment"], function (Pagination,moment) {
        	...
        	page = new Pagination({
              url: "${ctx}/brand/dealer/listDealerGroom/data",
              elem: "#pagination",
