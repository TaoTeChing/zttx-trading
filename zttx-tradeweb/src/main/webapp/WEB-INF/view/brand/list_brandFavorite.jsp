	...
	<table id="tempTable" class="agency-seeking-table">
		...
		{{each rows}}
			<tr class="{{$index%2==0 ? '':'odd'}}">
				...
				<td><span class="agency-seeking-td-jppn">{{$value.shopNum}}</span></td>
				<td><span class="agency-seeking-td-class">{{$value.empNum}}</span></td>
	...
	<script>
	  ...
	  $(function () {
	    seajs.use(["pagination","moment"], function (Pagination,moment) {
        	...
        	page = new Pagination({
              url: "${ctx}/brand/dealer/favorite/data",
              elem: "#pagination",
