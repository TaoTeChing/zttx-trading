...
<%@ page language="java" contentType="text/html;charset=UTF-8"
         import="java.util.Date,com.zttx.web.module.brand.model.BrandesAuthUserModel" pageEncoding="UTF-8" %>
<%@ page import="com.zttx.web.consts.DealerConstant" %>
<%@ page import="com.zttx.web.consts.BrandConstant" %>
<%@ page import="com.zttx.web.consts.BrandUsermConstant" %>
...
<div class="do-body">
    <div class="listbody clear">
        <div class="sidebar-l">
            <form:form action="" id="product_addcar_form">
                <div class="chanpin font">
                    <div class="magnibox f-l ad-gallery clearfix" style="position:relative;">
                        <div class="magnimg" style="position:relative;">
                            <a href="javascript:void(0)" class="jqzoom js-img-center">
                                ...
                                <img src="${res}${url}" data-bigimg="" title=""/>
                            </a>
                        </div>
                        <div class="magni-all">
                        ...
                        <div class="m-collect">
                            <c:if test="${isCollected}"><span
                                    class="yiguanzhu"></span>已收藏（${empty brandsCount ? 0 : brandsCount.favNum}）</c:if>
                            <c:if test="${not isCollected}"><a href="#" class="link"><i
                                    class="icon-collect"></i>收藏商品<span>（${empty brandsCount ? 0 : brandsCount.favNum}）</span></a></c:if>
                        </div>
                        <div class="m-share">
                    ...
                    <div class="checkbox f-l">
                        ...
                        <div class="m-detail">
                            ...
                            <dl class="clearfix">
                                <dt>起批量</dt>
                                <dd>>=${productInfo.productExtInfo.startNum}（<a href="#" class="link">起批说明</a>）</dd>
                            </dl>
                            <dl class="clearfix">
                                <dt>物流运费</dt>
                                <dd>
                           ...
                           <c:if test="${sizeIds}">
											...
											<ul class="m-colorlist inline-float" id="js-colorlist">
												<c:forEach  items="${colors}" var="item" varStatus="status">
															<c:if test="${status.index==0}">
																<li class="selected" ><a href="javascript:;">${item}</a><span></span></li>
															</c:if>
															<c:if test="${status.index!=0}">
								...
								<div class="m-detail">
									<dl class="clearfix">
										...
										<dd style="height:200px;overflow: auto;">
											  <c:forEach  items="${skuMap}" var="item" varStatus="status">
														<table class="table-list" <c:if test="${status.index!=0}">style="display:none;"</c:if>>
															<colgroup>
																<col width="90"/>
																<col width="95"/>
																<c:if test="${productInfo.productExtInfo.credit}">
															...
															<tbody id="js-list">
																	<c:forEach items="${item.value}" var="item2">
																			<tr>
																				<td style="font-weight: bold;"><input type="hidden" name="sku_ids" value="${item2.refrenceId}"/>${item2.productSkuName}</td>
																				<td><em class="js-p-xk">${item2.productSkuPrice.directPrice}</em>元</td>
																				<c:if test="${productInfo.productExtInfo.credit}">
																					<td><em class="js-p-sx">${item2.productSkuPrice.creditPrice}元</em></td>
																				</c:if>
																				<td>${item2.quantity}</td>
																				<td>
																					<div class="clearfix"><input type="text" class="num-amount" name="sku_amounts" value="0" data-max="${item2.quantity}"/></div>
																				</td>
																			</tr>
																	</c:forEach>
															</tbody>
													...
													<c:forEach  items="${skuMap}" var="item" varStatus="status">
														<c:forEach items="${item.value}" var="item2">
																<tr>
																	...
																	<td><em class="js-p-xk">${item2.productSkuPrice.directPrice}</em>元</td>
																	<c:if test="${productInfo.productExtInfo.credit}">
																		<td><em class="js-p-sx">${item2.productSkuPrice.creditPrice}</em>元</td>
																	</c:if>
																	<td>${item2.quantity}</td>
																	<td>
																		<div class="clearfix"><input type="text" class="num-amount" name="sku_amounts" value="0" data-max="${item2.quantity}"/></div>
                            ...
                            <div class="m-balance">
                                ...
                                <label>
                                	<c:if test="${productInfo.productExtInfo.credit}">
                                        <input type="radio" name="balances" value="1"/>授信价：<em id="js-sxprick">0</em>元
                                    </c:if>
                                </label>
                            </div>
            ...
            <c:if test="${sample}">
                <div class="chanpin-fd clearfix">
                    <table>
                        ...
                        <tbody>
                        <tr>
                            <td class="ta-c"><i class="icon-nayang"></i>拿样价:￥<fmt:formatNumber value="${samplePrice}" type="currency" pattern="0.00"/>
                            </td>
                            <td class="ta-l">
                                <p>产品：拿样产品仅支持拿样一件</p>
        ...
        <div class="sidebar-r">
            ...
            <div class="m-info">
                <%--未授权 开始--%>
                <c:if test="${null eq join}">
                    <div class="mi-fore5">
                        <a href="#">提交加盟申请书</a>
                    </div>
                </c:if>
                <%--未授权 结束--%>
                <c:if test="${null ne join}">
                    <div class="mi-fore1">
                        <dl class="clearfix">
            ...
            <div class="m-contact">
                ...
                <%--<div class="mc-fore2">
                    <p class="p1">${contact.userTelphone}</p>

                    <p>+86 ${contact.userMobile}</p>
                </div>
                --%><%--未授权 开始--%>
                <%--<div class="mc-fore2">
                        ...
                        <c:forEach items="${online.detailList}" var="detail">
                            <c:if test="${online.showed==1}">
                                <li>${detail.name}：<a
                                        ...
                                        src="${res}/images/market/qq-service.png" alt="" width="75"/></a></li>
                            </c:if>
                        </c:forEach>
                    </ul>
                </div>
            ...
            <div class="m-down">
                ...
                <ul class="inline-float">
                    <c:forEach items="${braList}" var="brandes">
                        <li>
                            <a href="http://${domain}${zttx}" class="js-img-center"><img src="${res}${brandes.brandLogo}" alt="" width="70"
                                                                   height="35"/></a>

                            <p><a href="http://${domain}${zttx}">${brandes.brandsName}</a></p>
                        </li>
                    </c:forEach>
                </ul>
            </div>
...
<script src="${src}/fronts/market/last.js"></script>
...
<script type="text/javascript">
    ...
    function toBalance(productId,productType)
    {
        var skuIds = [];
        var amounts = [];
  ...
  function applyJoin(productId){
    ...
    seajs.use(['template', 'underscore'], function (Template, _) {
        ...
        function countTwoPrice() {
            var _c_count = 0, _c_xk = 0, _c_sx = 0;

            $(".table-list tr").each(function () {
                var _p_xk = parseFloat($(this).find(".js-p-xk").text());     //现款价
                var _p_sx = parseFloat($(this).find(".js-p-sx").text());	  //授信价
                var _count = parseFloat($(this).find(".num-amount").val());  //数量
