    ...
    <div class="main">
        <div class="ts-container" style="background-color: #fff;">
            <div class="order-all">
                ...
                <form id="singleCreadeOrder" action="">
                        ...
                        <div class="m-uselist">
                                    ...
                                    <div class="pic-box clearfix">
                                        <p class="fl" style="margin-left: 0px;">
                                            <a target="_blank" href="/market/product/${dealerShoper.productId}">
                                                <img src="${res}${fns:getImageDomainPath(dealerShoper.productImage,120,120)}" title="${dealerShoper.productTitle}" width="120" height="120">
                                            </a>
                                ...
                                <div class="m-detail fr">
                                    <div class="o-hd clearfix">
                                        ...
                                        <div class="operate fr clearfix">
                                            <input type="hidden" name="productId" value="${dealerShoper.productId}"/>
                                            <input type="hidden" name="productType" value="${dealerShoper.productType}"/>
                                            ...
                                            <c:set var="point" value="<%=DealerConstant.DealerShoper.PRODUCTTYPE_POINT%>" /><!--返点-->
                                            交易类型：
                                            <c:choose>
                                                ...
                                                <c:when test="${dealerShoper.productType eq point}">
                                                    <i class="icon-fd" data-type="${point}"></i>返点</a>
                                                </c:when>
                                            </c:choose>
                                        </div>
                                    </div>
                                    <div class="o-bd attr-item">
                                        <table class="">
                                            ...
                                            <c:forEach var="dealerShopers" items="${dealerShoper.dealerShopersList}" varStatus="state">
                                                <tr>
                                                    <td class="p-colorsize">${dealerShopers.productSkuName}
                                                        <input type="hidden" name="skuIds" value="${dealerShopers.productSkuId}"/>
                                                    </td>
                                                    <c:if test="${dealerShoper.isPoint}">
                                                        <td class="p-price" ><fmt:formatNumber value="${dealerShopers.productSkuPrice}" type="currency"/>(<fmt:formatNumber value="${dealerShoper.pointPercent*100}" type="currencySymbol" pattern="#.#"/>%)</td>
                                                    </c:if>
                                                    <c:if test="${!dealerShoper.isPoint}">
                                                        <td class="p-price" ><fmt:formatNumber value="${dealerShopers.productSkuPrice}" type="currency"/></td>
                                                    </c:if>
                                                    <td class="p-number" >${empty dealerShopers.purchaseNum ? '0': dealerShopers.purchaseNum}
                                                        <input type="hidden" name="purchaseNum" value="${empty dealerShopers.purchaseNum ? '0': dealerShopers.purchaseNum}"/>
                                                    </td>
                                                    ...
                                                    <c:if test="${dealerShoper.isPoint}">
                                                        <td class="p-count js-price-single" ><fmt:formatNumber value="${(dealerShopers.productSkuPrice * (1 - dealerShoper.pointPercent)) * dealerShopers.purchaseNum}" type="currency" pattern="0.00"/></td>
                                                    </c:if>
                                                    <c:if test="${!dealerShoper.isPoint}">
                                    ...
                                    <div class="o-fd clearfix">
                                        <div class="count fr js-count-total" data-productid="${dealerShoper.productId}" data-productcount="${dealerShoper.productNum}">
                                            ...
                                            <c:if test="${!dealerShoper.isPoint}">
                                            <span>折扣优惠：<em><fmt:formatNumber value="${dealerShoper.discountPrice}" type="currency" pattern="0.00"/></em></span>
                                            </c:if>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="m-bottom clearfix mt10">
                                ...
                                <div class="fr">
                                    ...
                                    <c:if test="${!dealerShoper.isPoint}"></c:if>
            ...
            <div class="m-balabce">
                <div class="bd">
                    ...
                    <p class="tip"><span>（其中：授信产品货款</span><em class="js-allsx">0.00</em><span>元 现款产品货款</span><em class="js-allxk">0.00</em><span>元 返点产品货款</span><em class="js-allfd">0.00</em>)</p>
                    <p>
                        <c:if test="${_noShoper}">
                            <a class="link" href="${ctx}/market/product/${_productId}">返回商品详情页</a>
                        </c:if>
                        <c:if test="${!_noShoper}">
                            <a class="link" href="${ctx}/dealer/dealerShoper/cart">返回进货单修改</a>
                        </c:if>
                        <button class="btn create_order_js" onclick="shopper_balance.confirmCreateOrder()" data-flag="${_noShoper}">提交订单并支付</button>
                    </p>
                </div>
            </div>
        </div>
...
<script>
        ...
        initAddress:function(){
            ...
            var item = $(".address-item").click(function(){
                dialogLoading(function(_d){
                    shopper_balance.calcFreight(_d);
                }, "正在切换运费模板，请稍候...");
            });
            ...
            seajs.use("dialog",function(Dialog){
                $(function(){
                    var code = "${errCode}";
                    if(code && zttx.SUCCESS!=code){
                        /*var _noShoper = ${_noShoper};
                        var _a = _noShoper ? "<a class='link' href='${ctx}/market/product/${_productId}'>返回商品详情页</a>" : "<a class='link' href='${ctx}/dealer/dealerShoper/cart'>返回进货单修改</a>";
                        var _htm = '<div><div class="ui-head"><h3>提示</h3></div><div class="confirm_bd ta-c" style="margin-top: 28px;font-size:14px;">${message}，'+_a+'</div></div>';
                        new Dialog({
                            content: _htm,
                            width: 500
                        }).show();*/
                        $(".create_order_js").prop('disabled', true);
    ...
    /**计算运费 by JuST4iT*/
    shopper_balance.calcFreight=function(_loadingDialog){
        ...
        $('.order-item').each(function(){
            ...
            var productCnts=[];
            var $this = $(this);
            var rebateType = $this.find(".icon-fd").eq(0).data("type");  //3 为返点类型， 因为订单做了合并，所有只要查第一个类型
            ...
            if(area!=null){
                ...
                $.post("${ctx}/dealer/dealerShoper/calcfreight", params, function(data){
                    ...
                    if(data.code==126000){
                        var html=[];
                        if(rebateType == 3){
                            html.push('<p class="type">运送方式：<label>' +
                            '<input class="ui-radio" type="radio" name="frePayType_' + data.object + '" ' +
                            'value="60" ' +
                            'data-amount="0" checked>' +
                            '品牌商承担运费</label></p>');
                        }else{
                            if(data.rows==null||data.rows.length==0){
                                html.push('<p>运费模板不存在,请与品牌商联系</p>');
                            }else{
                        ...
                        $('#freigth_' + data.object).empty().append(data.message + ",请与品牌商联系");
                    }
                    if(_loadingDialog){
                        _loadingDialog.hide();
                    }
        ...
        var total = 0, xkTotal = 0, sxTotal = 0, fdTotal = 0;
        ...
        $(".js-allfd").html(fdTotal.toFixed(2));
    ...
    shopper_balance.confirmCreateOrder = function() {
        ...
        if(flag&&shopper_balance.setFreightRemark()){
            var prams = $("#singleCreadeOrder").serialize() + "&" +$("#order_comfirm").serialize();
            $.post("${ctx}/dealer/dealerOrder/addOrderNoShoper", prams,function(data){
                if(data.code==126000){
