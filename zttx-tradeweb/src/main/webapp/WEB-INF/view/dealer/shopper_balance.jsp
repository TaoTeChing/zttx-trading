...
<body>
	<div class="container">
				...
				<div class="main-right">
					...
					<div class="main-grid mb10">
						<div class="panel-crumbs">
							<i class="icon i-setting"></i><a href="${ctx }/dealer/center">管理中心</a> >> <a href="${ctx}/dealer/shopper" title="">我的进货单</a> > <span
								class="bb">订单结算</span> <a class="panel-crumbs-help" href="${ctx}/help/index"><i class="icon i-help"></i>帮助中心</a>
						</div>
					</div>
					<div class="main-grid mb10">
						<div class="panel">
									...
									<c:forEach items="${dealerAddrs}" var="dealerAddr">
										<li <c:if test="${dealerAddr.dealerDefault}">class="hover"</c:if> id="li_${dealerAddr.refrenceId}">
											<div>
												...
												<span class="address">${dealerAddr.provinceName}&nbsp;${dealerAddr.cityName}&nbsp;${dealerAddr.areaName}<span title='${dealerAddr.dealerAddress}'>${fns:trimLongString(dealerAddr.dealerAddress, 20)}</span>&nbsp;(${dealerAddr.dealerName })&nbsp;
					...
					<div class="main-grid mb10 purchases-table">
											...
											<c:if test="${product.buyTotal!=0 }">
												<tr>
													...
													<td align="center" valign="middle" style="padding: 10px;line-height: inherit;border-right: 1px solid #d9d9d9;"><img
														src="${res}${product.domainName}${fns:getImageDomainPath(product.image,220, 220)}" width="60px" height="60px" /> <br />
													<a href="#">货号:${product.pNo}</a>
																						...
																						<c:if test="${att_son.buyCount!=null&&att_son.buyCount!=0}">
																							...
																							<tr>
																								<td style="text-align:left">
                                                                                                    ...
                                                                                                    <c:if test="${fn:length(product_item_image_url)>6 }">
																								...
																								<td><fmt:formatNumber value="${att_son.price}" type="currency" pattern="0.00" /></td>
																								<td>${att_son.buyCount}</td>
																								<td><fmt:formatNumber value="${att_son.price*att_son.buyCount}" type="currency" pattern="0.00" /></td>
																							</tr>
																						</c:if>
																						<%--
																						<c:if test="${att_son.buyCount==null||att_son.buyCount==0}">
													...
													<td><fmt:formatNumber value="${product_money}" type="currency" pattern="0.00" /></td>
												</tr>
											</c:if>
										</c:forEach>
									</tbody>
						...
						<div class="pur-total">
							<div class="pur-address">
								...
								<p id='person_show_p'>收货人:${defaultAddr.dealerName}&nbsp;${empty defaultAddr.dealerMobile?'':defaultAddr.dealerMobile}${empty defaultAddr.dealerTel?'':' '}  ${empty defaultAddr.dealerTel?'':defaultAddr.dealerTel}  ${empty defaultAddr.postCode?'':' '} ${defaultAddr.postCode }</p>
							</div>
							...
							<div style="overflow:hidden;">
                             ...
                             <div class="bgc-ch ta-r">进货总量:<span class="c-r fs18 yahei bb">${product_total_count }</span> 件 | 总货款(不含运费)：￥<span class="c-r fs18 yahei bb"><fmt:formatNumber value="${product_total_money}" type="currency" pattern="0.00" /></span> <a class="ui-button ui-button-lred yahei fs18"
		...
		<div style="display: none;">
			...
			<form:form id="order_payment" action="${ctx}/dealer/order/payment" >
	...
	<script>
		...
		purchasesAddrss.setDisplayAddress = function(address) {
			...
			var personInfo = "收货人:" + address.data("dealername") + " "+ address.data("dealermobile")+ '&nbsp;'+ address.data("dealertel")+'&nbsp;'+ address.data("postcode");
		...
		$(function() {
			//注册设置默认地址事件
			$(document).on("click", "span.fr a.defadd",purchasesAddrss.setDealerAddrDefault);
			$('#Edit_Address').on("click",
					"input.ui-button.ui-button-mred.cancel",
					purchasesAddrss.closeAddressDlg);
			$('#order_submit').on("click",function(){

                var totalNum =parseFloat($(this).prev().text());

                if(totalNum <= 0){
                    remind("error","总货款不能为零");
                }else if(totalNum > 100000000){
                    remind("error","总货款不能超过一亿");
                }else{
                    purchasesAddrss.confirmCreateOrder();
                }
