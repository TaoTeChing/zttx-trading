            ...
            <div class="main-right">
                ...
                <div class="main-grid mb10">
                    <div class="panel-crumbs">
                        <i class="icon i-setting"></i><a href="${ctx}/dealer/center">管理中心</a> &gt;&gt;我的授权产品库&gt;&gt;
                        <span class="bb">我的浏览记录</span>
                    ...
                    <div class="main-grid mb20 clearfix">
                        <div class="panel-tablink clearfix">
                            <ul class="clearfix">
                                ...
                                <li><a href="${ctx}/dealer/dealerShoper/grant">我的授权产品库</a></li>
                                <li><a href="${ctx}/dealer/dealerShoper/orderc">我的常进款式</a></li>
                                <li><a href="${ctx}/dealer/dealerShoper/browseHistroy" class="on">我的浏览记录</a></li>
                        ...
                        <div class="panel-tbar clearfix">
                            <div class="search-bar">
                                <div class="search-box">
                                    <form:form id="filter-form" action="" onsubmit="return false;">
                                        <ul class="searchul">
                                           <%-- <li>
                                            ...
                                            <li class="select_li">
                                                货号：<input type="text" class="ui-input" name="productNo" value="" style="padding-left:5px;background:none;width: 280px;">
                                            </li>
                                            <li>
...
<script id="products-template" type="text/html">
    <ul class="product-item">
        ...
        {{each rows}}
        <li class="{{if $value.productCate==2}}preordered{{/if}} {{$index%4==0?'np':''}}" >
            ...
            <dl>
                <dt class="item-pic js-img-center {{if $value.type == 0}}illegal{{else if $value.type == 1}}down{{else if $value.type == 2}}illegal{{/if}}">
                    ...
                    <a class="btn-joinorder {{if $value.type == 0 || $value.type == 1 || $value.type == 2}}cantuse{{/if}}" data-product-id="{{$value.productId}}" data-product-type="{{if isCredit}}1{{else}}0{{/if}}" data-histroy-id="{{$value.productViewLogId}}" href="javascript:;">加入进货单</a>
                </dt>
                ...
                <dd class="item-name">
                    <h3 class="fn-text-overflow" title="{{$value.productNo}}"><a href="${ctx}/market/product/{{$value.productId}}" target="_blank" class="link">货号:{{$value.productNo}}</a>
                    </h3>
                </dd>
...
<script type="text/javascript">
    seajs.use(["${jsrel}/dealer/purchases"], function(Purchase){
        Purchase.init({
            url: "/dealer/dealerShoper/browseHistroy/data"
        });
        /**
         * 删除单个产品
         */
        $(document).on("click", ".delete-icon", function(){
            ...
            var productViewLogsId = parents.find(".btn-joinorder").attr("data-histroy-id");
            $.ajax({
                url: "${ctx}/dealer/dealerShoper/removeHistory",
                data: {productViewLogsId:productViewLogsId},
                dataType: "json",
                method: "post",
                success: function(){
                    g_pagination.goTo(1);
