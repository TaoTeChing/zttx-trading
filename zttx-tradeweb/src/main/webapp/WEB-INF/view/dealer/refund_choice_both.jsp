    ...
    <div class="em100">
        <div class="main clearfix pr">
            ...
            <div class="main-right">
                ...
                <div class="main-grid mb10">
                    <div class="panel-crumbs">
                        <i class="icon i-setting"></i><a href="${ctx}/dealer/center">管理中心</a> >> <a
                            href="${ctx}/dealer/dealerOrder/order" title="">已进货的订单</a> > <span class="bb">退款退货申请</span>
                        ...
                        <div class="top clearfix">
                            ...
                            <div class="left">
                                <div class="order_info">
                                    ...
                                    <p>
                                        <label for="">订单号：</label>
                                        <a href="${ctx}/dealer/dealerOrder/orderDetail/${dealerOrder.refrenceId}" target="_blank"
                                           class="link">
                                    ...
                                    <p>
                                        <label class="fl">品牌商：</label><span class="fl"
                                                                            style="width: 154px;">${dealerOrder.brandName}</span>
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="detail_box clearfix">
                                <!--选其一-->
                                <div class="fl mt20" style="width:99%;">
                                    <form:form id="refundApplyForm" class="ui-form"
                                               action="${ctx}/dealer/refund/apply/submit?received=true" method="post">
                                        <input type="hidden" name="adjustFreight"
                                               value="${dealerOrder.adjustFreight}"/><!-- 运费 -->
                                        <input type="hidden" name="orderNumber"
                                               value="${dealerOrder.orderId}"/><!-- 订单编号 -->
                                        <input type="hidden" name="needRefund"
                                               value="${dealerRefund.needRefund}"/><!-- 是否退货 -->
                                        <!--<div> 品牌商已发货，物流公司为【顺丰快递】，运单号【2154213】，联系品牌商(效果未完成) </div>-->
                                        <c:if test="${dealerRefund.updateNum != 0 && null != dealerRefund.updateNum}">
                                            <div class="payrecord">
                                                <p>
                                                    您还有 <strong>${3-dealerRefund.updateNum }</strong>
                                                    次修改退款退货申请的机会，请如实提交退款信息，您也可以要求客服介入
                                                </p>
                                            </div>
                                        </c:if>
                                        <div class="ui-form-item" style="height: 35px;">
                                            <label class="ui-label">退款原因：<span class="ui-form-required">*</span></label>
                                            <select class="ui-select" style="padding:5px;width:120px;"
                                                    name="refundReason" dataType="*">
                                                <option value="" ${dealerRefund.refundReason=='' ? 'selected':''}>请选择
                                                </option>
                                                <option value="退运费" ${dealerRefund.refundReason=='退运费' ? 'selected':''}>
                                                    退运费
                                                </option>
                                                <option value="收到商品破损" ${dealerRefund.refundReason=='收到商品破损' ? 'selected':''}>
                                                    收到商品破损
                                                </option>
                                                <option value="商品错发/漏发" ${dealerRefund.refundReason=='商品错发/漏发' ? 'selected':''}>
                                                    商品错发/漏发
                                                </option>
                                                <option value="商品需要维修" ${dealerRefund.refundReason=='商品需要维修' ? 'selected':''}>
                                                    商品需要维修
                                                </option>
                                                <option value="发票问题" ${dealerRefund.refundReason=='发票问题' ? 'selected':''}>
                                                    发票问题
                                                </option>
                                                <option value="收到商品与描述不符" ${dealerRefund.refundReason=='收到商品与描述不符' ? 'selected':''}>
                                                    收到商品与描述不符
                                                </option>
                                                <option value="商品质量问题" ${dealerRefund.refundReason=='商品质量问题' ? 'selected':''}>
                                                    商品质量问题
                                                </option>
                                                <option value="未收到货" ${dealerRefund.refundReason=='未收到货' ? 'selected':''}>
                                                    未收到货
                                                </option>
                                                <option value="未按约定时间发货" ${dealerRefund.refundReason=='未按约定时间发货' ? 'selected':''}>
                                                    未按约定时间发货
                                                </option>
                                                <option value="收到假货" ${dealerRefund.refundReason=='收到假货' ? 'selected':''}>
                                                    收到假货
                                                </option>
                                            </select>
                                        </div>
                                        <div class="ui-form-item">
                                            ...
                                            <%--<input class="ui-input" style="width:200px" name="refundAmount"
                                                   value="${dealerRefund.refundAmount}" dataType="money"/>--%>
                                            <input class="ui-input" style="width:200px" name="refundAmount"
                                                   value="${dealerOrder.productPrice + dealerOrder.adjustPrice +
                                            dealerOrder.adjustFreight}" dataType="money"/>
                ...
                <textarea class="ui-textarea" style="width:500px;height:60px;resize: none;" placeholder="描述您退款的详细理由"
                          dataType="*"
                          name="refundMark">${dealerRefund.refundMark}</textarea>
                                                ...
                                                <c:forEach items="${dealerRefAttachts}" var="dealerRefAttacht">
                                                    <li class="item">
                                                        <div class="img_contain">
                                                            <img src="${res}${dealerRefAttacht.domainName}${dealerRefAttacht.attachtName}"
                                                                 style="width:100px;height:100px;" alt=""><span></span>
                                                        </div>
                                                    </li>
                                                </c:forEach>
                                        ...
                                        <div class="ui-form-item">
                                            <button id="refundApplyFormSubmit" class="ui-button ui-button-lred">提交申请
                                            </button>
                                        </div>
                                    </form:form>
                                </div>
...
<div class="hide">
    <div id="close_refund">
        ...
        <div class="ta-c mb10">
            <a href="Purchases-refund-2-close.jsp" class="ui-button ui-button-morange">确定</a><a
                class="ui-button ui-button-morange ml20">取消</a>
        </div>
