                    ...
                    <div class="main-grid mb20 clearfix">
                        <div class="panel-tablink clearfix">
                            <ul class="clearfix">
                                ...
                                <li><a href="${ctx}/dealer/dealerFavorite/productFavorite" class="on">我的产品收藏夹</a></li>
                            </ul>
                        </div>
        ...
        <li class="pr {{if $value.productCate==2}}preordered{{/if}} {{$index%4==0?'np':''}}" >
            ...
            <dl>
                <dt class="item-pic js-img-center {{if $value.type == 0}}illegal{{else if $value.type == 1}}down{{else if $value.type == 2}}illegal{{/if}}">
                    ...
                    <a class="btn-joinorder {{if $value.type == 0 || $value.type == 1 || $value.type == 2}}cantuse{{/if}}" data-product-id="{{$value.productId}}" data-product-type="{{if isCredit}}1{{else}}0{{/if}}" data-favorite-id="{{$value.refrenceId}}" href="javascript:;">加入进货单</a>
                </dt>
...
<script type="text/javascript">
    seajs.use(["${jsrel}/dealer/purchases"], function(Purchase){
        Purchase.init({
            url: "/dealer/dealerFavorite/productFavorite/data",
            method:"post"
        });
        /**
         * 删除单个产品
        ...
        $(document).on("click", ".delete-icon", function(){
            ...
            var dealerFavoriteId = parents.find(".btn-joinorder").attr("data-favorite-id");
            $.ajax({
                url: "${ctx}/dealer/dealerFavorite/removeFavorite",
                data: { refrenceId : dealerFavoriteId},
                dataType: "json",
                method: "post",
                success: function(){
                    g_pagination.goTo(1);
