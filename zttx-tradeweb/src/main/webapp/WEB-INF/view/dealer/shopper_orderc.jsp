...
<head>
    ...
    <title>管理中心 &gt;&gt; 我的授权产品库 &gt;&gt;我的常进款式</title>
                    ...
                    <div class="main-grid mb20 clearfix">
                        <div class="panel-tablink clearfix">
                            <ul class="clearfix">
                                ...
                                <li><a href="${ctx}/dealer/dealerShoper/orderc" class="on">我的常进款式</a></li>
        ...
        <li class="{{if $value.productCate==2}}preordered{{/if}} {{$index%4==0?'np':''}}" >
            ...
            <dl>
                <dt class="item-pic js-img-center {{if $value.type == 0}}illegal{{else if $value.type == 1}}down{{else if $value.type == 2}}illegal{{/if}}">
                    ...
                    <a class="btn-joinorder {{if $value.type == 0 || $value.type == 1 || $value.type == 2}}cantuse{{/if}}" data-product-id="{{$value.productId}}" data-product-type="{{if isCredit}}1{{else}}0{{/if}}" data-orderc-id="{{$value.dealerOrdercId}}" href="javascript:;">加入进货单</a>
                </dt>
...
<script type="text/javascript">
    seajs.use(["${jsrel}/dealer/purchases"], function(Purchase){
        Purchase.init({
            url: "/dealer/dealerShoper/orderc/data"
        });
        /**
         * 删除单个产品
         */
        $(document).on("click", ".delete-icon", function(){
            ...
            var dealerOrdercsId = parents.find(".btn-joinorder").attr("data-orderc-id");
            console.log(dealerOrdercsId)
            $.ajax({
                url: "${ctx}/dealer/dealerShoper/removeOrderc",
                data: {"dealerOrdercsId":dealerOrdercsId},
                dataType: "json",
                method: "post",
                success: function(){
                    g_pagination.goTo(1);
