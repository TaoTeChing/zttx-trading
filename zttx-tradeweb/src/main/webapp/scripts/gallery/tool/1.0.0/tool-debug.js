define("gallery/tool/1.0.0/tool-debug", [ "gallery/underscore/1.6.0/underscore-debug", "arale/dialog/1.2.5/dialog-debug", "gallery/template/1.0.1/template-debug" ], function(require, exports, module) {
    var _ = require("gallery/underscore/1.6.0/underscore-debug");
    var Dialog = require("arale/dialog/1.2.5/dialog-debug");
    var template = require("gallery/template/1.0.1/template-debug");
    /**
    ...
    */
    var ZTTX = {
        ...
        unparam: function(str) {
            var vars = {}, hash, i, strParams = str.indexOf("?") > -1 ? str.split("?")[1] : str;
        ...
        _remindFn: function(type, message, timeout, fn) {
            ...
            var format = '<div style="border: #bababa solid 1px;background: #fff;box-shadow: 0 0 3px rgba(0,0,0,.4);">' + '<div style="font-size: 18px;line-height: 42px;padding: 10px 15px;">' + '<i style="display: inline-block;height: 42px;margin-right: 10px;vertical-align: middle;width: 42px;{{style}}"></i>' + "<span>{{message}}</span>" + "</div>" + "</div>";
        ...
        confirmDialog: function(config) {
            ...
            var reg = /<([^>]*)>/;
            //判断content有没有html标签，没有的话，则认为是简单的提示
            if (!reg.test(content)) {
                content = '<p class="ta-c mt20 fz14">' + content + "</p>";
            }
            var html = '<div id="confirmDialog_' + id + '">' + '<div class="ui-head"><h3>' + title + "</h3></div>" + '<div class="confirm_bd">' + content + '<div class="operate"><a class="confirm_btn" href="javascript:;">确定</a>';
        ...
        baseCalendar: function(config) {
            seajs.use([ "my97DatePicker" ], function() {
                ...
                $(config.elem).on("click", function() {
                    if (config.fromNow) {
                        var newjson = $.extend(config.json || {}, {
                            minDate: "%y-%M-%d"
                        });
                        WdatePicker(newjson);
                    } else {
        ...
        rangeCalendar: function(startConfig, endConfig, className) {
            ...
            seajs.use([ "my97DatePicker" ], function() {
                ...
                $("#" + endConfig.elem).on("focus", function() {
                    if (endConfig.endNow) {
                        WdatePicker({
                            minDate: "#F{$dp.$D('" + startConfig.elem + "')}",
                            maxDate: "%y-%M-%d"
                        });
                    } else {
                        WdatePicker({
        ...
        previewPic: function(json) {
            ...
            if (window.navigator.userAgent.indexOf("MSIE") >= 1) {
                ...
                $(json.previewBox).html(strHTML);
            } else {
                if (typeof FileReader === "undefined") {
                    alert("监测到您的浏览器不支持FileReader对象!");
                    return;
                }
                var reader = new FileReader();
                reader.readAsDataURL(json.f[0]);
