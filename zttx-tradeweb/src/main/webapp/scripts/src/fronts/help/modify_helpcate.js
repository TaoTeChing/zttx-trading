seajs.use(['$', 'widget', "dialog", "ztree"], function ($, Widget, Validator, Dialog) {

    baseFormValidator({
        selector: '#modify_help_form',
        isAjax: true,
        beforeSubmitFn: function(){
            $.post("/fronts/helpCate/save", $("#modify_help_form").serialize(), function (retData) {
                if (retData.code == 126000) {
                    window.location.href = "/fronts/helpInfo/list";
                }else {
                    remind("error", "编辑失败");
                }
            }, 'json');
    ...
    $.post("/fronts/helpCate/tree", null, function (retData) {
