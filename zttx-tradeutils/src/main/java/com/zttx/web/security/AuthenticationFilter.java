...
public class AuthenticationFilter extends org.apache.shiro.web.filter.authc.FormAuthenticationFilter
{
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception
    {
        ...
        { // 未登陆状态
            ...
            {// ajax请求
                ...
                JsonMessage message = getJsonMessage(CommonConst.USER_NOT_LOGIN);
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue)
    {
        ...
        if (!subject.isAuthenticated() && subject.isRemembered())
        {
            ...
            if (null != object)
            {
                HttpServletRequest httpRequest = (HttpServletRequest) request;
                HttpServletResponse httpResponse = (HttpServletResponse) response;
                UserPrincipal userInfo = (UserPrincipal) object;
                if(StringUtils.isBlank(userInfo.getUserPass())){//表明这是之前的用户COOKIE
                    // 清理自定义会话
                ...
                UsernamePasswordToken token = new UsernamePasswordToken(userInfo.getUserMobile(), userInfo.getUserPass().toCharArray(), true,
                        IPUtil.getOriginalIpAddr(httpRequest), null);
                subject.login(token);
            }
        }
        return subject.isAuthenticated();
     ...
     * @return
     */
    JsonMessage getJsonMessage(EnumDescribable describable)
    {
