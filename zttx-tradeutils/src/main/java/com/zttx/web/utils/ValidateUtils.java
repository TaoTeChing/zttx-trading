...
public class ValidateUtils
{
    ...
    public static boolean isDateFormat(String checkString, String dateFormat, boolean isRequired)
    {
        ...
        if (!boolNull)
        {
            ...
            if (dateFormat.equalsIgnoreCase(ApplicationConst.DATE_FORMAT_YM)) { return matchRegexp(checkString, "^([0-9]{4}-[0-9]{1,2})?$"); }
            if (dateFormat.equalsIgnoreCase(ApplicationConst.DATE_FORMAT_YMD)) { return matchRegexp(checkString, "^([0-9]{4}-[0-9]{1,2}-[0-9]{1,2})?$"); }
        }
        return true;
    }
